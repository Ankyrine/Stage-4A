	...
	try {
		...
		if($db != null) {
			...
			if($consult_date == "Donnée actuelle") {
				...
				$query = "SELECT nbr_baies_possible, nbr_baies_installable, room_name FROM rooms";
				...
				$query = "SELECT archives_room_nbr_baies_possible AS nbr_baies_possible, archives_room_nbr_baie_installable AS nbr_baies_installable, archives_room_room_name AS room_name FROM archives_rooms WHERE archives_room_date LIKE :consult_date";
			...
			if(count($datas) > 0) {
				...
				foreach($datas as $data) {
					...
					$room_value[] = $data['nbr_baies_possible'];
					$room_value_2[] = $data['nbr_baies_installable'];
					$room_name[] = $data['room_name'];
				}    

				/* Create and populate the pData object */ 
				$MyData = new pData();   
				$MyData->addPoints($room_value, "Nombre de baie possible"); 
				$MyData->setAxisName(0, "Nombre de baie possible");
				$MyData->addPoints($room_name, "Salles"); 
				$MyData->setAbscissa("Salles");
				$MyData->addPoints($room_value_2, "Nombre de baie installable");
