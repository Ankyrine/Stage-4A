...
<?php
	...
	/* Cette page permet l'affichage  et la consultation
	de la liste des fichiers présent dans le répertoire 
	de logs utilisateur */
	...
	if($user->userAuth() == 2) { // Niveau d'utilisation Administrateur de l'application
		...
		if(isset($_GET['log'])) { // si la variable $_GET a été transmise sur user_log.php

			$logfile = $_GET['log']; // on récupère la variable GET

			if(file_exists("./user_log/$logfile")) { // On vérifie l'existance du fichier

				$content = file_get_contents("./user_log/$logfile"); // S'il existe on récupère son contenu
			...
			if($dir = opendir('./user_log')) { // On ouvre le dossier des logs utilisateur
				...
				while(false !== ($logfile = readdir($dir))) { // Tant qu'il y a un fichier "non lu" dans le répertoire
					...
					if($logfile != '.' && $logfile != '..' && $logfile != 'index.php') { // Et que ce fichier n'est pas un lien relatif du dossier lui même, et n'est pas l'index du dossier
						...
						echo "<li><a href=\"user_logs.php?log=" . $logfile . "\">" . $logfile . "</a></li>"; // On liste chaque élément du dossier (chaque log du répertoire)
					}
				}
